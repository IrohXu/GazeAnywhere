...
def load_model(config_file, model_weights):
    ...
    if model is None:
        ...
        if not os.path.exists(model_weights):
            print(f"Model weights not found at {model_weights}. Attempting to download from Hugging Face...")
            try:
                import huggingface_hub
                hf_token = os.environ.get("HF_TOKEN")
                if not hf_token:
                    print("Warning: HF_TOKEN environment variable not set. Download may fail if the repository is gated.")
                
                # Ensure the checkpoints directory exists
                os.makedirs(os.path.dirname(model_weights), exist_ok=True)
                
                downloaded_path = huggingface_hub.hf_hub_download(
                    repo_id="IrohXu/GazeAnywhere",
                    filename="model.safetensors",
                    token=hf_token
                )
                
                import shutil
                shutil.copy(downloaded_path, model_weights)
                print(f"Successfully downloaded weights to {model_weights}")
            except Exception as e:
                raise gr.Error(f"Failed to download model weights: {e}. Please ensure you have access to IrohXu/GazeAnywhere and have set the HF_TOKEN environment variable.")
        try:

...
class AnyGazeModelMapper(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.linear = nn.Linear(1024, self.dim)
        self.linear_txt = nn.Linear(2048, self.dim)
    ...
    def forward(self, x):
        ...
        # Get out-dict
        image_features, text_features, _, patch_tokens, backbone_patch_tokens, text_patch_tokens = self.backbone(
            scenes,
            texts,
        )
        
        # Apply patch weights to get the final feats and attention maps
        img_feats = patch_tokens
        feats = self.linear(img_feats)
        txt_feats = self.linear_txt(text_features)
        
        txt_feats = torch.randn(txt_feats.shape, device=feats.device)
        ...
        if self.inout:
            feats = torch.cat([self.inout_token.weight.unsqueeze(dim=0).repeat(feats.shape[0], 1, 1), txt_feats.unsqueeze(-1).permute(0, 2, 1), feats], dim=1)
        ...
        if self.inout:
            ...
            feats = feats[:, 2:, :] # slice off inout tokens from scene tokens

...
if __name__ == "__main__":
    ...
    visualization_save_path = "output_gaze_video.mp4"
    ...
    frame_id_path = "/projects/illinois/eng/cs/jrehg/datasets-irb/devsci_autism/WeillCornell_PedabyteProject_processed/for_annotations/all/PWC007_2019_02_23_T1_panasonic_ESCS_merged_12129_15508/PWC007_2019_02_23_T1_panasonic_ESCS_merged_12129_15508_5fps_frames.txt"
    
    with open(frame_id_path, "r") as f:
        frame_id_df = pd.read_csv(f, sep="\t", header=None)
        frame_id_df = frame_id_df[0].tolist()
    
    # head_bbox_path = "/projects/illinois/eng/cs/jrehg/datasets-irb/devsci_autism/WeillCornell_PedabyteProject_processed/for_annotations/all/PWC007_2019_02_23_T1_panasonic_ESCS_merged_12129_15508/PWC007_2019_02_23_T1_panasonic_ESCS_merged_12129_15508_5fps_head_tracked_bboxes.txt"
    # with open(head_bbox_path, "r") as f:
    #     head_bbox_df = pd.read_csv(f, sep="\t", header=None)
    #     head_bbox_df = head_bbox_df[0].tolist()
    
    # head_bboxes = []
    # for bbox in head_bbox_df:
    #     bbox = bbox.split(",")
    #     bbox = [float(i) for i in bbox]
    #     head_bboxes.append(bbox)
        
    image_paths = "/projects/illinois/eng/cs/jrehg/datasets-irb/devsci_autism/WeillCornell_PedabyteProject_processed/for_annotations/all/PWC007_2019_02_23_T1_panasonic_ESCS_merged_12129_15508/clipped_frames"
    save_paths = "/projects/illinois/eng/cs/jrehg/users/xucao2/ChildGaze/tmp"
    
    for i in range(len(frame_id_df)):
        image_name = "0" * (4 - len(str(frame_id_df[i]))) + str(frame_id_df[i]) + ".jpg"
        image_path = os.path.join(image_paths, image_name)
        # subject_bbox = head_bboxes[i]
        subject_bbox = ovod_head_detection(image_path, processor, head_detection_model)
        if subject_bbox is None:
            print(f"Skipping frame {image_name} due to no detected head.")
            continue
        ...
        cv2.imwrite(os.path.join(save_paths, image_name), visualization_pred)
